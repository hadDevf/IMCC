// ============================================
// IMPORT FIREBASE SDK (STATIS - PASTI JALAN!)
// ============================================
import {
  initializeApp
...
// ============================================
// ENCRYPT/DECRYPT SESSION STORAGE
// ============================================
const SECRET_KEY = "imcc2026_rahasia_banget_123";

function encryptSession(value) {
  let result = '';
  for (let i = 0; i < value.length; i++) {
    result += String.fromCharCode(value.charCodeAt(i) ^ SECRET_KEY.charCodeAt(i % SECRET_KEY.length));
  }
  return btoa(result);
}

function decryptSession(encrypted) {
  try {
...
function setAdminSession(value) {
  const encrypted = encryptSession(value ? 'true': 'false');
  sessionStorage.setItem('isAdmin', encrypted);
}

function getAdminSession() {
  const encrypted = sessionStorage.getItem('isAdmin');
...
.then((userCredential) => {
  ...
  console.log("✅ Terhubung ke Firebase. UID:", currentUserUID);
  initializeAdminStructure().then(() => {
    checkAdminStatus();
  });
  const storedAdminStatus = getAdminSession();
...
async function initializeAdminStructure() {
  const adminsRef = ref(db,
    'admins');
  const snapshot = await get(adminsRef);
  if (!snapshot.exists()) {
    console.log('ℹ️ Struktur admin belum ada.');
  }
}
...
// ============================================
// VERIFY ADMIN PASSWORD (PAKE API)
// ============================================
...
function buildSocialIconsHTML(socials) {
  ...
  SOCIAL_PLATFORMS.forEach(platform => {
    if (socials[platform.key]) {
      html += `<a href="${socials[platform.key]}" target="_blank" class="w-8 h-8 rounded-lg bg-${platform.bgColor}-600/10 hover:bg-${platform.bgColor}-600/30 border border-${platform.color}-500/20 flex items-center justify-center ${platform.textColor} hover:text-white transition-all text-sm" title="${platform.label}"><i class="${platform.iconClass}"></i></a>`;
    }
  });
...
// ============================================
// UPLOAD MODAL FUNCTIONS (PAKE API VERCEL)
// ============================================
...
function renderCreators() {
  ...
  if (creatorsList.length === 0) {
    grid.innerHTML = `<div class="col-span-full text-center py-20 bg-white/[0.01] border border-dashed border-white/10 rounded-3xl"><i class="fa-regular fa-folder-open text-gray-600 text-5xl mb-4"></i><h3 class="text-lg font-bold text-gray-400">Belum ada creator terdaftar</h3><p class="text-gray-500 text-xs mt-2">Creator akan muncul di sini setelah ditambahkan melalui Panel Admin.</p></div>`;
    return;
  }
  creatorsList.forEach(creator => {
    ...
    const cardHTML = `<div class="creator-card glass-panel rounded-3xl overflow-hidden border border-white/5 transition-all duration-300 select-none" data-name="${creator.name}"><div class="h-28 w-full relative"><img src="https://i.urusai.cc/HlvoE.jpg" class="w-full h-full object-cover object-top absolute inset-0" alt="Minecraft Banner"><div class="absolute inset-0 bg-gradient-to-t from-mineDark-950/90 via-mineDark-950/20 to-transparent"></div></div><div class="relative px-6 -mt-10 z-10 flex justify-between items-end"><div class="w-20 h-20 rounded-2xl bg-mineDark-950 p-[3px] shadow-2xl border-2 border-white/10 overflow-hidden"><img src="${avatarImg}" onerror="this.src='https://placehold.co/120x120/141923/ffffff?text=MC'" alt="${creator.name}" class="w-full h-full rounded-2xl object-cover"></div><span class="text-[9px] uppercase font-bold tracking-widest px-2.5 py-1 rounded-full whitespace-nowrap mb-1 bg-diamond/10 text-diamond border border-diamond/20">Creator</span></div><div class="pt-4 px-6 pb-6 relative"><div><h3 class="text-lg font-bold text-white">${creator.name}</h3><p class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider mt-0.5">Minecraft Creator</p></div><p class="text-gray-400 text-xs mt-4 leading-relaxed h-16 overflow-y-auto pr-1">${creator.bio || 'Tidak ada deskripsi'}</p><div class="flex flex-wrap items-center gap-2 mt-5 py-3 border-t border-white/5">${socialsHTML}</div></div></div>`;
...
function renderAdminRegistrations() {
  ...
  if (registrationsList.length === 0) {
    list.innerHTML = `<div class="text-center py-12 bg-white/[0.01] border border-dashed border-white/5 rounded-2xl"><i class="fa-solid fa-circle-check text-gray-600 text-3xl mb-3"></i><p class="text-xs text-gray-500 font-bold uppercase">Antrean Bersih</p><p class="text-[10px] text-gray-600 mt-1">Tidak ada pengajuan pendaftaran baru.</p></div>`;
    return;
  }
  registrationsList.forEach((reg, index) => {
    const avatarIcon = reg.avatarUrl ? `<img src="${reg.avatarUrl}" class="w-8 h-8 rounded-full object-cover border border-white/20 mr-2" />`: '';
    const itemHTML = `<div class="glass-panel p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all space-y-3 relative overflow-hidden"><div class="absolute right-0 top-0 text-[50px] font-bold text-white/[0.02] font-mono pointer-events-none select-none">#${index + 1}</div><div class="flex justify-between items-start"><div class="flex items-center">${avatarIcon}<div><h4 class="text-sm font-bold text-white font-mono">${reg.name}</h4><p class="text-[10px] text-emeraldGlow font-bold mt-0.5"><i class="fa-solid fa-square-phone mr-1"></i>${reg.phone}</p></div></div><span class="text-[8px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-diamond/10 text-diamond border border-diamond/20">Creator</span></div><p class="text-[11px] text-gray-400 bg-mineDark-950 p-3 rounded-lg border border-white/[0.02]">${reg.details || '<span class="text-gray-600 italic">Tanpa info detail opsional</span>'}</p><div class="flex gap-2 pt-1"><button onclick="approveRegistration('${reg.firebaseKey}')" class="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-black text-[10px] font-extrabold rounded-lg tracking-wider transition-colors uppercase font-mono"><i class="fa-solid fa-check mr-1"></i> Terima</button><button onclick="rejectRegistration('${reg.firebaseKey}')" class="px-4 py-1.5 bg-red-500/10 hover:bg-red-500/30 border border-red-500/30 text-red-400 hover:text-white text-[10px] font-bold rounded-lg tracking-wider transition-colors uppercase font-mono"><i class="fa-solid fa-trash mr-1"></i> Tolak</button></div></div>`;
...
function renderAdminCreators() {
  ...
  tableBody.innerHTML = '';
  if (creatorsList.length === 0) {
    tableBody.innerHTML = `<tr><td colspan="4" class="py-8 text-center text-gray-500 italic">Tidak ada creator terdaftar di database.</td></tr>`;
    return;
  }
  creatorsList.forEach(creator => {
    const avatarImg = creator.avatar || 'https://placehold.co/120x120/141923/ffffff?text=' + creator.name.substring(0, 2).toUpperCase();
    const platformsHTML = buildAdminSocialIconsHTML(creator.socials);
    const rowHTML = `<tr class="hover:bg-white/[0.02] transition-colors border-b border-white/5"><td class="py-3 pl-2 flex items-center gap-3"><img src="${avatarImg}" onerror="this.src='https://placehold.co/120x120/141923/ffffff?text=MC'" class="w-8 h-8 rounded-xl object-cover border border-white/10" /><div><h4 class="font-bold text-white font-mono">${creator.name}</h4><span class="text-[8px] bg-diamond/10 text-diamond border border-diamond/20 px-1.5 py-0.5 rounded">Creator</span></div></td><td class="py-3 hidden md:table-cell text-gray-400 max-w-xs truncate" title="${creator.bio}">${creator.bio || 'Tidak ada deskripsi'}</td><td class="py-3 hidden sm:table-cell"><div class="flex items-center gap-2 text-sm">${platformsHTML}</div></td><td class="py-3 text-right pr-2"><div class="flex items-center justify-end gap-2"><button onclick="openEditModal('${creator.firebaseKey}')" class="px-2.5 py-1 border border-diamond/30 hover:border-diamond bg-diamond/5 hover:bg-diamond/10 text-diamond text-[10px] font-bold rounded-lg transition-colors"><i class="fa-solid fa-user-gear"></i> Edit</button><button onclick="deleteCreator('${creator.firebaseKey}')" class="px-2.5 py-1 border border-red-500/30 hover:border-red-500 bg-red-500/5 hover:bg-red-500/10 text-red-400 hover:text-white text-[10px] font-bold rounded-lg transition-colors"><i class="fa-solid fa-trash-can"></i> Hapus</button></div></td></tr>`;
...
function deleteCreator(firebaseKey) {
  ...
  .then(() => alertNotification('error', `Creator "${creator.name}" telah dihapus secara live.`))
  .catch((err) => alertNotification('error', `Gagal menghapus creator: ${err.message}`));
}
...
function handleEditCreatorSubmit(event) {
  ...
  const updatedData = {
    ...
    avatar: document.getElementById('editAvatar').value.trim() || `https://placehold.co/120x120/141923/00F5FF?text=${document.getElementById('editName').value.substring(0, 2).toUpperCase()}`,
  ...
  .catch((err) => alertNotification('error', `Gagal memperbarui profil: ${err.message}`));
}
...
function handleFormSubmit(event) {
  ...
  .catch((err) => alertNotification('error', `Gagal mengirim pendaftaran: ${err.message}`));
}
...
function handleDirectCreatorUpload(event) {
  ...
  .catch((err) => alertNotification('error', `Gagal menambahkan creator: ${err.message}`));
}
...
function approveRegistration(firebaseKey) {
  ...
  .then(() => {
    ...
    .then(() => alertNotification('success', `Pendaftaran "${reg.name}" disetujui & otomatis dipublikasikan secara live!`));
  })
  .catch((err) => alertNotification('error', `Gagal memproses persetujuan: ${err.message}`));
}
...
function rejectRegistration(firebaseKey) {
  ...
  if (!reg) return;
  remove(ref(db, 'registrations/' + firebaseKey))
  .then(() => alertNotification('error', `Pendaftaran "${reg.name}" berhasil ditolak.`))
  .catch((err) => alertNotification('error', `Gagal menghapus antrean: ${err.message}`));
}
...
async function updateActivePage() {
  ...
  document.querySelectorAll('.nav-pill').forEach(pill => {
    pill.className = "nav-pill px-5 py-2.5 rounded-xl text-xs font-bold tracking-widest border transition-all duration-300 uppercase flex items-center gap-2 text-gray-300 border-white/5 bg-white/[0.02]";
  });
  ...
  document.querySelectorAll('.mobile-nav-pill').forEach(pill => {
    pill.className = "mobile-nav-pill flex items-center justify-between px-5 py-4 rounded-xl text-sm font-semibold tracking-wider text-gray-300 bg-mineDark-900/90 border border-white/5 transition-all uppercase";
  });
...
function alertNotification(type, message) {
  ...
  toast.className = `glass-panel ${borderColor} border-l-4 p-4 rounded-xl shadow-2xl flex items-center space-x-3 w-80 translate-y-10 opacity-0 transition-all duration-300 z-50`;
  toast.innerHTML = `<i class="fa-solid ${icon} text-lg"></i><div class="flex-1"><p class="text-xs font-semibold text-white">${message}</p></div><button onclick="this.parentElement.remove()" class="text-gray-500 hover:text-white transition-colors"><i class="fa-solid fa-xmark text-xs"></i></button>`;
